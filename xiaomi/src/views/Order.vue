<template>
  <div data-v-bc186c1a class="app-view-wrapper">
    <!---->
    <div data-v-172bb70c data-v-bc186c1a class="content app-view app-view-with-footer">
      <header data-v-172bb70c class="hd">
        <router-link to="/signin" data-v-172bb70c class="user ui-flex align-center">
          <div data-v-172bb70c class="img">
            <img data-v-172bb70c src="../assets/avatar.76a75b8f17.png">
          </div>
          <div data-v-172bb70c class="name">登录/注册</div>
        </router-link>
        <!---->
      </header>
      <div data-v-172bb70c class="b1 ui-flex align-center justify-space-between">
        <div data-v-172bb70c class="cite">我的订单</div>
        <div data-v-172bb70c class="span ui-flex align-center">
          <a
            data-v-172bb70c
            href="https://m.mi.com/order/list"
            class
            data-stat-id="a7eaff9f80200456"
          >全部订单</a>
        </div>
      </div>
      <ul data-v-172bb70c class="b2 ui-flex align-center justify-space-between">
        <li data-v-172bb70c class="dfk">
          <a
            data-v-172bb70c
            href="https://m.mi.com/order/list?type=7"
            class
            data-stat-id="511ab24289b98fe6"
          >
            <div data-v-172bb70c class="icon"></div>
            <span data-v-172bb70c>待付款</span>
            <!---->
          </a>
        </li>
        <li data-v-172bb70c class="dah">
          <a
            data-v-172bb70c
            href="https://m.mi.com/order/list?type=8"
            class
            data-stat-id="7ed7f427a4d20235"
          >
            <div data-v-172bb70c class="icon"></div>
            <span data-v-172bb70c>待收货</span>
            <!---->
          </a>
        </li>
        <li data-v-172bb70c class="thx">
          <a data-v-172bb70c href="https://m.mi.com/aftersale/list" data-stat-id="497872313157d22e">
            <div data-v-172bb70c class="icon"></div>
            <span data-v-172bb70c>退换修</span>
          </a>
        </li>
      </ul>
      <div data-v-172bb70c class="ui-line"></div>
      <ul data-v-172bb70c class="items">
        <li data-v-172bb70c class="i-member">
          <a
            data-v-172bb70c
            href="https://m.mi.com/mpointsmall"
            class
            data-stat-id="a8133610f1c901ef"
          >
            <cite data-v-172bb70c>会员中心</cite>
          </a>
        </li>
        <li data-v-172bb70c class="i-wallet">
          <a
            data-v-172bb70c
            href="https://m.mi.com/user/coupon"
            class
            data-stat-id="30d39469c9f405e6"
          >
            <cite data-v-172bb70c>我的优惠</cite>
          </a>
        </li>
      </ul>
      <div data-v-172bb70c class="ui-line"></div>
      <ul data-v-172bb70c class="items">
        <li data-v-172bb70c class="i-service">
          <a data-v-172bb70c href="https://m.mi.com/services" class data-stat-id="e90b943993e0cce3">
            <cite data-v-172bb70c>服务中心</cite>
          </a>
        </li>
        <li data-v-172bb70c class="i-mihome">
          <a
            data-v-172bb70c
            href="https://m.mi.com/maplocation/address?location_type=mihome"
            class
            data-stat-id="f6b73f8a7216172d"
          >
            <cite data-v-172bb70c>小米之家</cite>
          </a>
        </li>
      </ul>
      <div data-v-172bb70c class="ui-line"></div>
      <ul data-v-172bb70c class="items">
        <li data-v-172bb70c class="i-fcode">
          <a data-v-172bb70c href="https://m.mi.com/fcode" class data-stat-id="84aaa6155f11abc1">
            <cite data-v-172bb70c>F码通道</cite>
          </a>
        </li>
      </ul>
      <div data-v-172bb70c class="ui-line"></div>
      <ul data-v-172bb70c class="items">
        <li data-v-172bb70c class="i-setting">
          <a data-v-172bb70c href="https://m.mi.com/user/set" class data-stat-id="36040021395921ae">
            <cite data-v-172bb70c>设置</cite>
          </a>
        </li>
      </ul>
    </div>
  </div>
</template>
<script>
export default {};
</script>
<style scoped>
        .content[data-v-172bb70c] {
            background: #f5f5f5 !important
        }

        .hd[data-v-172bb70c] {
            background: url(/static/img/bg.63c8e19851.png) center 0 #f37d0f;
            background-size: auto 100%;
            padding: .32rem 0
        }

        .hd .img[data-v-172bb70c] {
            margin: 0 .16rem 0 .32rem;
            width: .96rem;
            height: .96rem;
            overflow: hidden;
            box-sizing: border-box;
            border-radius: 100%;
            border: 3px solid hsla(0, 0%, 100%, .4);
            text-align: center
        }

        .hd .img img[data-v-172bb70c] {
            width: auto;
            height: 100%;
            margin: 0 auto
        }

        .hd .name[data-v-172bb70c] {
            color: #fff;
            font-size: .24rem;
            text-align: left
        }

        .hd .account[data-v-172bb70c] {
            color: hsla(0, 0%, 100%, .6)
        }

        .b1[data-v-172bb70c] {
            background-color: #fff;
            height: .8rem;
            border-bottom: 1px solid rgba(0, 0, 0, .15);
            color: rgba(0, 0, 0, .54);
            position: relative;
            padding-right: .56rem
        }

        .b1[data-v-172bb70c]:after {
            content: "";
            position: absolute;
            right: 5.12px;
            right: .32rem;
            top: 50%;
            width: 3.2px;
            width: .2rem;
            height: 3.2px;
            height: .2rem;
            border-left: 1px solid currentColor;
            border-top: 1px solid currentColor;
            transform: translate3d(0, -50%, 0) rotate(135deg);
            -webkit-transform: translate3d(0, -50%, 0) rotate(135deg)
        }

        .b1 .cite[data-v-172bb70c] {
            font-size: .28rem;
            padding: 0 0 0 .32rem;
            color: rgba(0, 0, 0, .87)
        }

        .b1 .span[data-v-172bb70c],
        .b1 a[data-v-172bb70c] {
            font-size: .24rem;
            color: rgba(0, 0, 0, .87)
        }

        .b2[data-v-172bb70c] {
            padding: 0 .8rem;
            background-color: #fff
        }

        .b2 li[data-v-172bb70c] {
            display: -webkit-box;
            display: -ms-flexbox;
            display: -webkit-flex;
            display: flex;
            -webkit-align-content: space-between;
            align-content: space-between;
            position: relative;
            padding: .4rem 0
        }

        .b2 li .icon[data-v-172bb70c] {
            width: .48rem;
            height: .48rem;
            margin: 0 auto .16rem;
            background: url(data:image/png;base64,) no-repeat center 0;
            background-size: 100% auto
        }

        .b2 li span[data-v-172bb70c] {
            color: rgba(0, 0, 0, .87)
        }

        .b2 li.dah .icon[data-v-172bb70c] {
            background-image: url(data:image/png;base64,)
        }

        .b2 li.thx .icon[data-v-172bb70c] {
            background-image: url(/static/img/nav-4.d68723895f.png)
        }

        .b2 li em[data-v-172bb70c] {
            position: absolute;
            min-width: .28rem;
            line-height: .28rem;
            height: .28rem;
            box-sizing: border-box;
            padding: 0 .05rem;
            font-size: .18rem;
            overflow: hidden;
            text-align: center;
            border-radius: .28rem;
            background: #ed4d41;
            color: #fff;
            right: 0;
            top: 0;
            transform: translate3d(50%, -50%, 0);
            -webkit-transform: translate3d(50%, -50%, 0);
            top: 50%;
            left: 50%;
            right: auto;
            -webkit-transform: translate3d(.1rem, -200%, 0);
            transform: translate3d(.1rem, -200%, 0);
            font-style: normal
        }

        .items[data-v-172bb70c] {
            background: #fff
        }

        .items li[data-v-172bb70c] {
            position: relative;
            color: rgba(0, 0, 0, .54);
            background: url(/static/img/i-setting.fb9625b3f2.png) no-repeat .32rem;
            background-size: .48rem auto;
            border-bottom: 1px solid rgba(0, 0, 0, .15);
            line-height: 0;
            list-style: none
        }

        .items li[data-v-172bb70c]:not(:first-child):before {
            content: "";
            position: absolute;
            left: 0;
            top: -2px;
            height: 3px;
            background: #fff;
            width: 17.92px;
            width: 1.12rem
        }

        .items li[data-v-172bb70c]:after {
            content: "";
            position: absolute;
            right: 5.12px;
            right: .32rem;
            top: 50%;
            width: 3.2px;
            width: .2rem;
            height: 3.2px;
            height: .2rem;
            border-left: 1px solid currentColor;
            border-top: 1px solid currentColor;
            transform: translate3d(0, -50%, 0) rotate(135deg);
            -webkit-transform: translate3d(0, -50%, 0) rotate(135deg)
        }

        .items li[data-v-172bb70c]:last-child {
            border-bottom-color: #fff
        }

        .items li.i-member[data-v-172bb70c] {
            background-image: url(data:image/png;base64,)
        }

        .items li.i-wallet[data-v-172bb70c] {
            background-image: url(data:image/png;base64,)
        }

        .items li.i-service[data-v-172bb70c] {
            background-image: url(data:image/png;base64,)
        }

        .items li.i-more[data-v-172bb70c] {
            background-image: url(/static/img/i-more.9814de691d.png)
        }

        .items li.i-mihome[data-v-172bb70c] {
            background-image: url(data:image/png;base64,)
        }

        .items li.i-fcode[data-v-172bb70c] {
            background-image: url(data:image/png;base64,)
        }

        .items li.i-setting[data-v-172bb70c] {
            background-image: url(/static/img/i-setting.fb9625b3f2.png)
        }

        .items li>a[data-v-172bb70c] {
            display: block;
            padding-right: .56rem;
            padding-left: 1.12rem;
            height: 1.04rem;
            display: box;
            display: -webkit-box;
            box-align: center;
            -webkit-box-align: center;
            box-pack: justify;
            -webkit-box-pack: justify;
            color: currentColor
        }

        .items li>a cite[data-v-172bb70c] {
            font-size: .28rem;
            font-style: normal;
            color: rgba(0, 0, 0, .87)
        }
</style>
